...
export const generateVirtualTryOn = async (
  ...
  try {
    ...
    // FIX: Updated error message to remove reference to VITE_API_KEY.
    if (error instanceof Error && error.message.includes('API key not valid')) {
        throw new Error("Invalid API Key. Please check your API_KEY environment variable.");
    }
    throw new Error("Failed to generate virtual try-on image. The AI service might be unavailable or the request was rejected.");
  }
};
